    ...
    def perform_action(self, state, action):
        ...
        if action == SPIN_LEFT or action == SPIN_RIGHT:
            ...
            for i in range(self.n_widgets):
                if action == REVERSE and new_robot_posit in widget_cells[i]:
...
def widget_get_movement_type(robot_orient, forward_robot_posit, centre):
    ...
    if direction == robot_orient:
        return TRANSLATE
    elif (direction == {ROBOT_UP: ROBOT_DOWN_RIGHT,
                        ROBOT_UP_LEFT: ROBOT_UP_RIGHT,
                        ROBOT_DOWN_LEFT: ROBOT_UP,
                        ROBOT_DOWN: ROBOT_UP_LEFT,
                        ROBOT_DOWN_RIGHT: ROBOT_DOWN_LEFT,
                        ROBOT_UP_RIGHT: ROBOT_DOWN}[robot_orient]):
        return SPIN_CW
    else:
        return SPIN_CCW
